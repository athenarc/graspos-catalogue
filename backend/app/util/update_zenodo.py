...
async def update_records(user_id = None, zenodo_id = None):
    ...
    records = await Zenodo.get(zenodo_id) if zenodo_id else await Zenodo.find().to_list()
    print(records)
    updated = []
    for record in [records]:
        ...
        if data["zenodo_object"]["zenodo_id"] != record.zenodo_id:
            ...
            if not zenodo_record:
                return HTTPException(status_code=404, detail="Zenodo record does not exist")
            ...
            update = Update(user_id = user_id, zenodo_id = record.id, old_version = record.zenodo_id, new_version = data["zenodo_object"]["zenodo_id"] ) 
            await update.save()
            updated.append(update)
    
    detail = "Zenodo records updated successfully" if not zenodo_id else "Zenodo record " + zenodo_id + " updated successfully"
            
    return {"status": 200, "detail": detail, "records_updated": len(updated), "zenodo_id": zenodo_id, "user_id": user_id}
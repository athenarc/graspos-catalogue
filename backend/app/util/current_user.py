...
from jwt import access_security, user_from_credentials, optional_access_security

from fastapi import Security, HTTPException
from fastapi_jwt import JwtAuthorizationCredentials


async def current_user(auth: JwtAuthorizationCredentials
                       | None = Security(optional_access_security)) -> User | None:
...
async def current_user_mandatory(auth: JwtAuthorizationCredentials = Security(
    access_security)) -> User:
    """Return the current authorized user."""

    if not auth:

        raise HTTPException(401, "Token time expired: Signature has expired.")
